	...
	if filter_lookahead > 0 then
		...
		s_lookahead = ("Lookahead: %gs\n"):format(filter_lookahead)
			.. ("Slowdown ramp: %g + (time * %g) ^ %g\n"):format(
				opts.slowdown_ramp_constant,
				opts.slowdown_ramp_factor,
				opts.slowdown_ramp_exponent
			)
	else
...
local function update_info(opt_now)
	...
	if opts.infostyle == "total" or opts.infostyle == "compact" or opts.infostyle == "verbose" then
		...
		if
			...
			and saved_total + s.saved_current == 0
			and s.time == nil
		then
			return false
		end
...
local function check_time()
	...
	for index = events_ifirst, events_ilast do
		...
		if filter_lookahead > 0 then
			...
			if ev.is_silent and opts.minduration > 0 and not events[index + 1] then
				remaining_pts = math.max(remaining_pts, ev.pts + opts.minduration - input_pts)
			end
	...
	if is_silent then
		...
		if remaining > 0 then
			...
			local s = base_speed
				* (opts.ramp_constant + (stats_silence_length(now) * opts.ramp_factor) ^ opts.ramp_exponent)
...
local function handle_silence_msg(msg)
	...
	if msg.text:find("silencedetect: silence_", 1, true) ~= 1 then
		return
	end

	local startend, pts = msg.text:match("^silencedetect: silence_(%a+): ([0-9%.]+)")
	pts = tonumber(pts)
...
(require("mp.options")).read_options(opts, nil, function(list)
	...
	if
		...
		or list["slowdown_ramp_constant"]
		or list["slowdown_ramp_factor"]
		or list["slowdown_ramp_exponent"]
		or list["minduration"]
		or list["startdelay"]
		or list["margin_start"]
		or list["margin_end"]
...
mp.add_key_binding("F3", "threshold-down", function()
	adjust_thresholdDB(-1)
end, "repeatable")
mp.add_key_binding("F4", "threshold-up", function()
	adjust_thresholdDB(1)
end, "repeatable")
